...
property_name = 'MasterUserPassword'
...
describe Object.const_get(rule_name(resource_type, property_name, sub_property_name)), :rule do
  ...
  password_rule_test_sets.each do |test_description, desired_test_result|
    context "#{resource_type} #{property_name} #{test_description}" do
      it context_return_value(desired_test_result) do
        run_test(resource_type, property_name, sub_property_name, test_template_type,
