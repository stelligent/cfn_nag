...
describe S3Util, :s3 do
  ...
  describe '#retrieve_s3_object_content_to_tmp_file' do
    ##
    # this doesn't cover missing bucket i don't think but.... maybe just let it rip
    # the SDK exceptions don't describe what is missing... so maybe better to rethrow?
    ##
    context 'when the object does not exist' do
      before(:each) do
        @stubbed_s3.stub_responses(:get_bucket_location, {
      ...
      it 'raises an error' do
        expect {
          _ = @s3_util.retrieve_s3_object_content_to_tmp_file bucket_name: 'arealbucket',
    ...
    context 'when the object exists in bucket' do
      before(:each) do
        ...
        @stubbed_s3.stub_responses(:get_object, {
          body: 'fredfredfred'
        })
      end

      it 'writes the object to a local tmp file' do
        actual_temp_file_path = @s3_util.retrieve_s3_object_content_to_tmp_file bucket_name: 'bucket_with_zips',
