  ...
  describe '#retrieve_files_within_input_artifact' do
    context 'when there is one s3 zip artifact with matching glob' do
      it 'returns contents of matched files within the zip' do
        ...
        expect(CodePipelineUtil).to receive(:retrieve_input_artifact)
                                .and_return 'spec/test_templates/json_templates.zip'
        expect(CodePipelineUtil).to receive(:cleanup) { }

        actual_file_contents = CodePipelineUtil.retrieve_files_within_input_artifact codepipeline_event: code_pipeline_event
  ...
  describe '#retrieve_input_artifact' do
    context 'when there is one s3 artifact' do
      it 'retrieves the one artifact' do
        ...
        expect(s3util_double).to receive(:retrieve_s3_object_content_to_tmp_file)
                             .with(bucket_name: 'somebucket', object_key: 'somekey')
                             .and_return expected_zip_path

        expect(CodePipelineUtil).to receive(:s3util)
                                .and_return(s3util_double)

        actual_zip_path = CodePipelineUtil.retrieve_input_artifact({
                                                   'data' => {
                                                     'inputArtifacts' => [
                                                       {
                                                         'location' => {
    ...
    context 'when is there is zero, or > 1 artifact' do
      it 'raises an error' do
        ...
        }.to raise_error 'Must have 1 and only 1 input artifact'
      end
    end
    ...
    context 'when is one non-s3 artifact' do
      it 'raises an error' do
        ...
        }.to raise_error 'S3 is only type supported'
      end
    end
  end
end
